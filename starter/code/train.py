...
def get_trainer(args, model, loss_fn, optimizer):
    def update_model(trainer, batch):
        model.train()
        optimizer.zero_grad()
        net_inputs, target, test_time_target = prepare_batch(args, batch, model.vocab)
        y_pred, test_time_res = model(**net_inputs)
        batch_size = y_pred.shape[0]
        loss, stats = loss_fn(y_pred, target),
        test_time_loss, test_time_stats = loss_fn(test_time_res, test_time_target)
        loss += test_time_loss
        loss.backward()
        optimizer.step()
