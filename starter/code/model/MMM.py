...
# Drama QA inserted with test time learning
class DMM(nn.Module):
    def __init__(self, args, vocab, n_dim, image_dim, layers, dropout, num_choice=5):
        ...
        self.embedding = nn.Embedding(V, D) #V: # of vocabs, dimensions per vocab
        n_dim = args.n_dim
        ...
        self.lstm_script = RNNEncoder(321, 150, bidirectional=True, dropout_p=0, n_layers=1, rnn_type="lstm") ### we might not need this
        ...
        if self.vbb_on:
            ...
            self.test_time_linear = nn.Sequential(nn.Linear( 6 * 150 * (image_dim+21),4), nn.Softmax(dim=1))
    ...
    def forward(self, que, answers, **features):
        ...
        if self.script_on:
            ...
            o_s_ = self.stream_processor(self.classifier_script,self.mhattn_script, spk_flag, H_S, s_len, q_c, e_q, q_len, e_ans_list, ans_len)
        else:
            o_s = 0
        ...
        if self.vmeta_on:
            ...
            o_m,_ = self.stream_processor(self.classifier_vmeta, self.mhattn_vmeta, vp_flag, H_M, vmeta_len, q_c, e_q, q_len, e_ans_list, ans_len)
        else:
            o_m = 0
        ...
        if self.vbb_on:
            ...
            o_b, test_time_res = self.stream_processor(self.classifier_vbb, self.mhattn_vbb, vp_flag, H_B, vbb_len, q_c, e_q, q_len, e_ans_list, ans_len, name='vbb')
        ...
        return out.view(B, -1), test_time_res

       
    def stream_processor(self, classifier, mhattn, ctx_flag, ctx, ctx_l,
                         qa_character, q_embed, q_l, a_embed, a_l, name=None):
        ...
        if name is not None:
            test_time_input = [u_q, *u_a]
            res = self.test_time_linear(test_time_input)
        else:
            res = None
