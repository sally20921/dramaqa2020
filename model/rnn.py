...
class  Embedding(nn.Module):
  def __init__(self, word_embedding_size, hidden_size):
    ...
    self.word_embedding_size = torch.tensor(word_embedding_size, dtype = torch.long)
    self.hidden_size = torch.tensor(hidden_size, dtype=torch.long)
    self.dropout =  nn.Dropout(0.2)
    #self.hidden = torch.tensor(2, dtype=torch.float, device=torch.device('cuda'))
    self.proj = nn.Linear(self.word_embedding_size, self.hidden_size, bias=False)
    self.hwy = HighwayEncoder(2, 28)
    self.avgattn = AverageAttn(12)

  def forward(self, x):
   # emb = self.dropout(x)
    x = torch.tensor(x, dtype=torch.long, device =  torch.device('cuda'))
    #emb = self.proj(x)
    emb = x
    print(x.size())
    emb_avg = self.avgattn(x)
    emb_avg = torch.tensor(emb_avg, dtype=torch.long, device=torch.device('cuda'))
    emb = torch.cat((emb, emb_avg), dim=1)
    emb =  self.hwy(emb)
    return  emb
