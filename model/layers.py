...
class AverageAttn(nn.Module):
    ...
    def __init__(self, hidden_size):
        ...
        #self.avgatt = torch.tensor(self.avgatt, dtype=torch.long)
        
    def forward(self, x):
        m = x.size(0)
        n = 1
        #x = torch.tensor(x, dtype=torch.long)
        avgatt = torch.tensor(self.avgatt, dtype=torch.float).cuda()
        #avgatt = avgatt.long()
        x = x.float()
        att_score = torch.matmul(x, avgatt).squeeze()
        att_score = F.softmax(att_score)
        att_score = att_score.view(m, n, 1)
        x_score = x * att_score
        avg_x = torch.sum(x_score, dim=2)
...
class HighwayEncoder(nn.Module):
    ...
    def __init__(self,num_layers, hidden_size):
        ...
        t = torch.tensor(num_layers, dtype = torch.int)
        self.h = torch.tensor(hidden_size, dtype=torch.int, device=torch.device('cuda'))
        self.transform = nn.Linear(self.h, self.h, bias=False) 
        self.gate = nn.Linear(self.h, self.h, bias=False) 
    def forward(self, x):
        if True:
            # Shapes of g, t, and x are all (batch_size, seq_len, hidden_size)
            x = torch.tensor(x, dtype=torch.float, device=torch.device('cuda'))
            g = self.gate(x)
            g = torch.sigmoid(x)
            t = self.transform(x)
            t = F.relu(x)
...
class Encoder(nn.Module):
  def __init__(self, input_size, hidden_size, num_layers, drop_prob=0.):
    ...
    self.drop_prob = drop_prob
    self.rnn = nn.GRU(input_size, hidden_size, num_layers, batch_first=True, bidirectional=True, dropout=drop_prob)
...
class SelfAtt(nn.Module):
    ...
    def __init__(self, hidden_size, drop_prob):
        ...
        self.drop_prob = drop_prob
        self.att_wrapper = TimeDistributed(nn.Linear(hidden_size, hidden_size))
        self.trilinear = TriLinearAttention(hidden_size)
        self.self_att_upsampler = TimeDistributed(nn.Linear(hidden_size*3, hidden_size))
    ...
    def forward(self, att, c_mask):
        ...
        #(batch_size * c_len, 1600)
        print("before time distributed:", att.shape)
